      ...
      _export("ScrollView", ScrollView = (_dec = ccclass('ScrollView'), _dec2 = property(Prefab), _dec3 = property(Node), _dec(_class = (_class2 = class ScrollView extends Component {
        ...
        initItems() {
          ...
          for (var i = 0; i < this.itemCount; i++) {
            var item = instantiate(this.scrollItem);
        ...
        updateItemPositions() {
          var contentY = this.content.position.y;
          var offsetY = contentY - this.contentStartY; // 计算实际的滚动偏移量

          var scrollOffset = Math.abs(offsetY); // 计算当前第一个可见item的索引

          var firstVisibleIndex = Math.floor(scrollOffset / this.itemHeight);

          for (var i = 0; i < this.items.length; i++) {
            var _this$itemDataIndexMa;

            var item = this.items[i];
            var currentIndex = firstVisibleIndex + i; // 检查是否超出数据范围
            ...
            var posY = -currentIndex * this.itemHeight;
            item.position = new Vec3(item.position.x, posY, 0); // 获取当前item正在显示的数据索引

            var currentDataIndex = (_this$itemDataIndexMa = this.itemDataIndexMap.get(item)) != null ? _this$itemDataIndexMa : -1; // 只有当数据索引发生变化时才更新内容
        ...
        updateItemContent(item, index) {
          ...
          var scrollItem = item.getComponent(_crd && ScrollItem === void 0 ? (_reportPossibleCrUseOfScrollItem({
            error: Error()
